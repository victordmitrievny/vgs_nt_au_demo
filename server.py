...
VGS_MERCHANT_ID_AU = os.getenv('VGS_MERCHANT_ID_AU')
VGS_MERCHANT_ID_NT = os.getenv('VGS_MERCHANT_ID_NT')
...
def handle_request():
    if request.method == 'POST':
        ...
        #Enroll in Account Updater:
        account_updater = enroll_account_updater(card_holder, card_number_token, card_exp_month, card_exp_year, sa_token)
        if not account_updater:
            return {"status": "failed - account updater enrollment"}
        ...
        if not network_token:
            return {"status": "failed - network token enrollment"}
        ...
        if not process_payment:
            return {"status": "failed - process payment with Adyen"}
...
#Enroll card in Account Updater
def enroll_account_updater(card_holder, card_number_token, card_exp_month, card_exp_year, sa_token):
    url = "https://calm.sandbox.verygoodsecurity.app/cards"
...
def enroll_network_token(card_number_token, card_exp_month, card_exp_year, sa_token):
    ...
    payload = {
        ...
        "exp_month": card_exp_month,
        "exp_year": card_exp_year,
        "vgs_merchant_id": VGS_MERCHANT_ID_NT
    }
